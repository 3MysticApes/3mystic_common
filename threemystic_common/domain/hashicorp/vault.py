...
class hashi_vault(base): 
  ...
  def __authenticate_aws_iam(self, client:hasicorp_hvac.Client=None, data = None, mount_point = "aws", pull_ec2_meta = False, *args, **kwargs):
    ...
    if data is not None and not self.common.isNullOrWhiteSpace(data.get("vault_role")):
    ...
    try:
      self._get_client(client= client).auth.aws.iam_login(**self.common.merge_dictionary([{}, default_iam_auth_creds, self.__authenticate_get_credentials(pull_ec2_meta= pull_ec2_meta)]))
      return
    except Exception as ex_session:
      try:
        if not self.common.isNullOrWhiteSpace(os.environ.get("AWS_ACCESS_KEY_ID")) and not pull_ec2_meta:
          self._get_client(client= client).auth.aws.iam_login(**self.common.merge_dictionary([{}, default_iam_auth_creds, {
  ...
  def _get_secret_data(self, secret_data, secret_key = None, *args, **kwargs ):
    ...
    if self.common.isNullOrWhiteSpace(secret_key):
      return secret_data
  ...
  def delete_secret(self, secret_path, version = None, client = None, *args, **kwargs):
    ...
    if not self.common.is_type(version, list):
      version = [version]
